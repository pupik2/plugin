...
class logsHelper
{
    ...
    {
        if (!logsLicensing::check()->isPremium()) {
            return;
        }
    ...
    private static function getHighlightedString($string, $query)
    {
        ...
        return preg_replace(
            sprintf(
                '~(%s)~ui',
                wa_make_pattern(waString::escape($query))
            ),
            sprintf('<span %s>$1</span>', $style_attribute),
            waString::escape($string)
        );
    }
